...
import { isMoodEmotionId, moodEmotionsById, MoodEmotionId } from '@/src/data/moodEmotions';
...
import { tabBarHeight, scrollPastTabBar } from '@/src/theme/tabBar';
...
export default function MoodDetailScreen() {
  ...
  const emotion = isMoodEmotionId(emotionId) ? moodEmotionsById[emotionId] : undefined;
  const duaId = emotion?.duaId;
  const dua = duaId ? duasById[duaId] : undefined;
  const emotionLabel = emotion ? t.mood.emotions[emotion.id as MoodEmotionId] : '';
  ...
  if (!dua || !duaId || !emotion) {
    return (
      <SafeAreaView style={styles.safe}>
        ...
        <Text style={styles.notFound}>{t.mood.notFound}</Text>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.tapLayer}>
        ...
        <GestureDetector gesture={countGesture}>
          ...
          >
            <View style={styles.titleBlock}>
              <View style={[styles.emotionBadge, { borderColor: night?.line ?? colors.line, backgroundColor: night?.card ?? colors.card }]}>
                <GoalLibraryIcon name={emotion.icon} color={night?.muted ?? colors.olive} size={22} />
            ...
            <Card style={styles.duaCard}>
              ...
              {dua.hadithNarration ? (
                <View style={styles.hadithBlock}>
                  <Text style={styles.hadithHeading}>{t.mood.hadith}</Text>
                  <Text style={styles.hadithText}>{dua.hadithNarration}</Text>
                </View>
              ) : null}
            </Card>
        ...
        <View pointerEvents="none" style={[styles.feedbackLayer, { bottom: tabBarHeight(language) }]}>
          <DuaTapFeedback
...
function createStyles(
  ...
  return StyleSheet.create({
    ...
    hadithText: {
      ...
      fontSize: typo.small,
      fontStyle: proseFontStyle(language),
      lineHeight: Math.round(typo.small * 1.55),
      ...proseLayout(language),
    },
    counterBadge: {
      alignItems: 'baseline',
